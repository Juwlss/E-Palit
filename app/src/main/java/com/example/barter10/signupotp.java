...
public class signupotp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        textmobile.setText(String.format(
                "+63-%s", getIntent().getStringExtra("mobiles")
        ));
    ...
    private void addUser(){
        String fname = getIntent().getStringExtra("fullnamedb");
        String uname = getIntent().getStringExtra("usernamedb");
        String pass = getIntent().getStringExtra("passwordb");
        String pho = getIntent().getStringExtra("phoNodb");
        String pic = "gs://e-palit.appspot.com/PostItem/Default/Vector (1).png";
        String userID = firebaseAuth.getUid();

        String username = uname+"@epalit.com";

        firebaseAuth.createUserWithEmailAndPassword(username, pass).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()){
                    User user = new User(userID,fname,pic,uname,pass,pho);
